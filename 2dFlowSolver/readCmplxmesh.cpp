                    ...
                    vector< vector< vector<int> > > &bound)
{
  ...
  double *x=NULL;     // Real part of the x coordinate
  double *y=NULL;     // Real part of the y coordinate
  double *xIm=NULL;   // Imaginary part of the x coordinate
  double *yIm=NULL;   // Imaginary part of the y coordinate
  int    *nbs=NULL;   // number of edges for each boundary    nbs[nb]
  int    **tri=NULL;  // tri-node connectivity  [nt][3]
  int    **quad=NULL; // quad-node connectivity [nq][4]
  int    ***bs=NULL;  // nodes for each edge of each boundary bs[nb][nbs][2]
  ...
  if ((xIm = (double*)malloc(nn*sizeof(double))) == NULL)
  {
    printf("\nCould not allocate memory for xIm\n");
    exit(0);
  }
  if ((yIm = (double*)malloc(nn*sizeof(double))) == NULL)
  {
    printf("\nCould not allocate memory for yIm\n");
    exit(0);
  }
  ...
  {
    fgets(buff,bdim,fp);
    sscanf(buff,"%lg %lg %lg %lg",&x[n],&xIm[n],&y[n],&yIm[n]);
  }
